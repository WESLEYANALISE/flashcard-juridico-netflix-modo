...
import { TrendingUp, Target, Award, Clock, BookOpen, Brain, Zap, Trophy, AlertTriangle, CheckCircle2, Calendar, BarChart3 } from 'lucide-react';
...
const ImprovedStatsView = () => {
  ...
  const stats = useMemo(() => {
    ...
    const needsReview = userProgress.filter(p => p.needs_review).length;
    const mastered = userProgress.filter(p => p.mastery_level === 'mastered').length;
    const currentStreak = Math.max(...userProgress.map(p => p.streak_count), 0);

    // Category stats based on real data
    const categoryStats = areas.map(area => {
      const areaCards = supabaseFlashcards.filter(card => card.area === area);
      const areaProgress = userProgress.filter(p => p.area === area);
      ...
      const areaAccuracy = areaAttempts > 0 ? (areaCorrect / areaAttempts) * 100 : 0;
      const areaNeedsReview = areaProgress.filter(p => p.needs_review).length;
      const areaMastered = areaProgress.filter(p => p.mastery_level === 'mastered').length;
      ...
      return {
        ...
        studied: areaProgress.length,
        mastered: areaMastered,
        needsReview: areaNeedsReview,
        accuracy: Math.round(areaAccuracy),
        progress: areaCards.length > 0 ? (areaProgress.length / areaCards.length) * 100 : 0
      };
    });

    // Progress over time (last 7 days)
    const progressOverTime = Array.from({ length: 7 }, (_, i) => {
      const date = new Date();
      date.setDate(date.getDate() - i);
      const dayProgress = userProgress.filter(p => {
        const studiedDate = new Date(p.last_studied);
        return studiedDate.toDateString() === date.toDateString();
      }).length;
      
      return {
        date: date.toLocaleDateString('pt-BR', { weekday: 'short' }),
        cards: dayProgress,
        accuracy: dayProgress > 0 ? Math.round(Math.random() * 20 + 70) : 0 // Mock accuracy for visualization
      };
    }).reverse();
  ...
  }, [supabaseFlashcards, areas, userProgress]);
  ...
  const flashcardsForSummary = supabaseFlashcards.map(card => ({
    ...
    studied: userProgress.some(p => p.flashcard_id === parseInt(card.id)),
    correctAnswers: userProgress.find(p => p.flashcard_id === parseInt(card.id))?.correct_answers || 0,
    totalAttempts: userProgress.find(p => p.flashcard_id === parseInt(card.id))?.total_attempts || 0,
    lastStudied: undefined,
  }));

  const categoriesForSummary = generateCategoriesFromAreas(areas);
  ...
  // Data for charts
  const pieData = [
    { name: 'Dominados', value: stats.mastered, color: '#22C55E' },
    { name: 'Precisa Revisar', value: stats.needsReview, color: '#F59E0B' },
    { name: 'Não Estudados', value: stats.totalCards - stats.studiedCards, color: '#6B7280' },
  ];
    ...
    <div className="min-h-screen bg-netflix-black px-2 sm:px-4 py-4 sm:py-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          {/* Progress Over Time */}
          <Card className="bg-netflix-dark/50 border-white/10 p-6 glass-effect animate-fade-in" style={{ animationDelay: '0.5s' }}>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={stats.progressOverTime}>
                ...
                />
                <Line type="monotone" dataKey="cards" stroke="#E50914" strokeWidth={3} dot={{ fill: '#E50914', strokeWidth: 2, r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </Card>

          {/* Distribution Pie Chart */}
          <Card className="bg-netflix-dark/50 border-white/10 p-6 glass-effect animate-fade-in" style={{ animationDelay: '0.6s' }}>
            <h3 className="text-xl font-semibold text-white mb-6 flex items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 animate-fade-in" style={{ animationDelay: '0.7s' }}>
        ...
        <div className="mt-8 text-center animate-fade-in" style={{ animationDelay: '0.9s' }}>
          <Card className="bg-gradient-to-r from-netflix-red/20 to-netflix-gold/20 border-netflix-red/30 p-6 sm:p-8 glass-effect hover:scale-105 transition-all duration-300 cursor-pointer group">
            ...
            <p className="text-sm sm:text-base text-gray-300 max-w-2xl mx-auto">
              {stats.studiedCards > 0 ? (
                stats.needsReview > 0 
                  ? `Você tem ${stats.needsReview} cards para revisar e fortalecer seu conhecimento.` 
                  : 'Parabéns! Você dominou todos os cards estudados. Continue assim!'
              ) : 'Comece sua jornada de estudos! Selecione uma área para estudar.'}
            </p>
          </Card>
        </div>
      </div>
