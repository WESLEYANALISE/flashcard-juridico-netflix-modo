...
const ImprovedThemeSelector = ({
  ...
  const getThemeStats = (theme: string) => {
    ...
    return {
      ...
      accuracy: themeProgress.length > 0 ? Math.round(themeProgress.reduce((sum, p) => sum + p.correct_answers, 0) / themeProgress.reduce((sum, p) => sum + p.total_attempts, 0) * 100) || 0 : 0
    };
  };
  if (isLoading) {
  ...
  return <div className="min-h-screen bg-netflix-black px-2 sm:px-4 py-4 sm:py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8 py-[24px]">
          <Button onClick={onBack} variant="outline" className="bg-white/10 border-white/20 text-white hover:bg-white/20 flex items-center space-x-2">
        ...
        <div className="text-center mb-8 px-[6px]">
          ...
          <div className="flex justify-center space-x-4 mb-8">
            <Button onClick={handleSelectAll} variant="outline" size="sm" className="bg-white/10 border-white/20 text-white hover:bg-white/20">
              Selecionar Todos
            </Button>
            <Button onClick={handleClearAll} variant="outline" size="sm" className="bg-white/10 border-white/20 text-white hover:bg-white/20">
              Limpar Seleção
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          ...
          return <div key={theme} onClick={() => handleThemeToggle(theme)} className={`
                  relative p-6 rounded-xl cursor-pointer transition-all duration-300 hover-lift animate-fade-in
                  ${isSelected ? 'ring-2 shadow-2xl' : 'hover:scale-105'}
                `} style={{
            ...
            animationDelay: `${index * 0.1}s`
                ...
                <div className="absolute top-4 right-4">
                  ...
                  `}>
                    {isSelected && <CheckCircle className="w-4 h-4" style={{
                  color: areaColor
                }} />}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-400 mb-3">
                    ...
                    {stats.accuracy > 0 && <span>{stats.accuracy}% precisão</span>}
                  </div>
                </div>
                ...
                {/* Hover Effect */}
                <div className="absolute inset-0 opacity-0 hover:opacity-20 transition-opacity duration-300 rounded-xl pointer-events-none" style={{
        ...
        <div className="text-center">
          <Button onClick={handleStartStudy} disabled={selectedThemes.length === 0} className="bg-netflix-red hover:bg-netflix-red/80 text-white px-8 py-4 text-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2 mx-auto">
            <Play className="w-5 h-5" />
            <span>Iniciar Estudo ({selectedThemes.length} temas)</span>
          </Button>
          
          {selectedThemes.length === 0 && <p className="text-gray-400 text-sm mt-2">
